...
fun AuditLogged(spørreundersøkelseService: SpørreundersøkelseService) =
    createRouteScopedPlugin("AuditLogged") {
        ...
        pluginConfig.apply {
            onCallRespond { call ->
                ...
                when (Miljø.cluster) {
                    Cluster.`prod-gcp`, Cluster.`dev-gcp` -> auditLog.info(logstring)
...
private fun ApplicationCall.erSpørreundersøkelseRoute() = request.uri.indexOf(SPØRREUNDERSØKELSE_PATH, ignoreCase = true) >= 0
